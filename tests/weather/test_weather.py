...
# ---------------------------------------------------------------------------
# fetch_weather_iterative — fehlende Pfade aus Coverage-Report
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fetch_weather_iterative_empty_queries() -> None:
    """Test fetch_weather_iterative mit leeren initial_queries gibt [] zurück."""
    provider = FakeWeatherProvider()
    results = await fetch_weather_iterative(provider, [])
    assert results == []


@pytest.mark.asyncio
async def test_fetch_weather_iterative_single_iteration_max_iterations_1() -> None:
    """Test fetch_weather_iterative bricht bei max_iterations=1 nach erster Iteration ab.

    Deckt Zeile 88 (iteration == max_iterations - 1 break) ab.
    """
    provider = FakeWeatherProvider()
    now = datetime(2026, 8, 2, 10, 0)
    ...
    results = await fetch_weather_iterative(
        provider, queries, max_iterations=1, convergence_threshold_s=1800
    )
...
@pytest.mark.asyncio
async def test_fetch_weather_iterative_deviation_branch_hash_mod_3() -> None:
    """Test fetch_weather_iterative Abweichungsbranch (hash % 3 == 0).

    Deckt Zeilen 99-101 ab: neue Zeit = alte Zeit + delta bei hash % 3 == 0.
    """
    provider = FakeWeatherProvider()
    now = datetime(2026, 8, 2, 10, 0)

    # Koordinate mit hash % 3 == 0
    test_coord: Coordinate = (-10.0, -2.0)  # hash % 3 == 0
    queries = [WeatherQuery(koordinate=test_coord, zeitpunkt=now)]

    results = await fetch_weather_iterative(
        provider, queries, max_iterations=2, convergence_threshold_s=1800
    )

    assert len(results) == 1
    # Die test_coord hat hash % 3 == 0 → neue Zeit + 31 Min
    expected_new_time = now + timedelta(seconds=1860)  # 1800 + 60
    assert results[0].zeitpunkt == expected_new_time


@pytest.mark.asyncio
async def test_fetch_weather_iterative_no_deviation_branch() -> None:
    """Test fetch_weather_iterative Konvergenz ohne Abweichungen (else-Zweig).

    Deckt Zeilen 102-103 ab: new_time = sample.zeitpunkt wenn hash % 3 != 0.
    """
    provider = FakeWeatherProvider()
    now = datetime(2026, 8, 2, 10, 0)

    # BERLIN und HAMBURG haben hash % 3 != 0 (meistens)
    queries = [
        WeatherQuery(koordinate=BERLIN, zeitpunkt=now),
        WeatherQuery(koordinate=HAMBURG, zeitpunkt=now),
    ]
    ...
    assert len(results) == 2
    # Zeiten sollten unverändert geblieben sein (keine Abweichung)
    for sample in results:
        assert sample.zeitpunkt == now


@pytest.mark.asyncio
async def test_fetch_weather_iterative_convergence_break() -> None:
    """Test fetch_weather_iterative bricht bei Konvergenz vor max_iterations ab.

    Deckt Zeilen 111-120 ab: Abweichungsberechnung + max(deviations) < threshold.
    """
    provider = FakeWeatherProvider()
    now = datetime(2026, 8, 2, 10, 0)

    # Koordinaten ohne Abweichung (hash % 3 != 0)
    queries = [
        WeatherQuery(koordinate=BERLIN, zeitpunkt=now),
        WeatherQuery(koordinate=HAMBURG, zeitpunkt=now),
    ]

    # Mit max_iterations=2, aber keine Abweichungen → break nach Iteration 1
    results = await fetch_weather_iterative(
        provider, queries, max_iterations=2, convergence_threshold_s=1800
    )
...
@pytest.mark.asyncio
async def test_fetch_weather_iterative_max_iterations_reached() -> None:
    """Test fetch_weather_iterative läuft max_iterations durch bei Abweichungen.

    Deckt Zeilen 111-124 ab: volle Iteration mit Abweichungen und Update.
    """
    provider = FakeWeatherProvider()
    now = datetime(2026, 8, 2, 10, 0)

    # Koordinate mit hash % 3 == 0 → Abweichung
    test_coord: Coordinate = (-10.0, -2.0)  # hash % 3 == 0
    queries = [WeatherQuery(koordinate=test_coord, zeitpunkt=now)]
    ...
    # Mit max_iterations=2:
    # Iteration 0: fetch @ 10:00, neue Zeit = 10:31
    # Iteration 1: fetch @ 10:31, break (iteration == max_iterations - 1)
    # → Ergebnis hat Zeit 10:31 (einmal verschoben)
    assert len(results) == 1
    expected = now + timedelta(seconds=1860)  # 1800 + 60
    assert results[0].zeitpunkt == expected


@pytest.mark.asyncio
async def test_fetch_weather_iterative_multiple_queries_different_behavior() -> None:
    """Test fetch_weather_iterative mit Mix aus Abweichung/keine-Abweichung Koordinaten.

    Deckt alle Zweige in der Iterationsschleife ab.
    """
    provider = FakeWeatherProvider()
    now = datetime(2026, 8, 2, 10, 0)

    # (-10.0, -2.0) → hash % 3 == 0 → Abweichung
    # BERLIN → hash % 3 != 0 → keine Abweichung
    coord_deviation: Coordinate = (-10.0, -2.0)
    coord_no_deviation: Coordinate = BERLIN

    queries = [
        WeatherQuery(koordinate=coord_deviation, zeitpunkt=now),
        WeatherQuery(koordinate=coord_no_deviation, zeitpunkt=now),
    ]
    ...
    assert len(results) == 2
    dev_sample = next(s for s in results if s.koordinate == coord_deviation)
    no_dev_sample = next(s for s in results if s.koordinate == coord_no_deviation)

    # Abweichungskoorinaten: Zeit verschoben
    assert dev_sample.zeitpunkt != now
    # Keine-Abweichung-Koordinaten: Zeit gleich
    assert no_dev_sample.zeitpunkt == now
