...
def _make_segment(index: int, length_m: float = 50_000.0) -> RouteSegment:
        ...
        plan = ChargingPlan(
            ladehalte=[
                ChargingStop(
                    ...
                    departure_time=departure_time + timedelta(hours=1, minutes=30),
                )
            ],
            gesamtreisezeit_s=0,
        )
...
class TestStep9UpdateEtaWaypointWaits:
    ...
    def test_waypoint_wait_duration_added_after_its_segment(self) -> None:
        segments = [_make_segment(i) for i in range(3)]
        segment_eta = [(seg, timedelta(hours=1)) for seg in segments]
        arrival = departure_time + timedelta(hours=1)
    ...
    def test_downstream_eta_timestamp_includes_the_wait(self) -> None:
        ...
        eta_at_segment_2 = departure_time + result[0][1] + result[1][1]
        ...
        plan = ChargingPlan(
            ladehalte=[
                ChargingStop(
                    ...
                    departure_time=departure_time + timedelta(minutes=30),
                )
            ],
            gesamtreisezeit_s=0,
            zwischenstopp_aufenthalte=[
