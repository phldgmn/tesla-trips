...
def fake_provider() -> FakeConstructionProvider:
    """Erstelle FakeProvider mit Test-Baustellen."""
    zones = [
        ConstructionZone(
            betroffene_segmente=[0],
            tempolimit_kmh=80,
            sperrungstyp=Sperrungstyp.TEMPORARY_SPEED_LIMIT,
            umleitungshinweis="Baustelle, Vorsicht!",
            land=Land.DE,
...
async def test_fake_provider_returns_test_zones(
    ...
    """FakeProvider gibt vorkonfigurierte Test-Baustellen zurück."""
    route = Route(
        segments=[],
        gesamtlaenge_m=1.0,
        geometrie=[],
...
async def test_fake_provider_no_filter(
    ...
    """FakeProvider gibt alle Baustellen zurück wenn kein Land-Filter."""
...
def test_construction_zone_tempolimit_required_for_partial_closure() -> None:
    """Validierung: tempolimit_kmh required für PARTIALLY_CLOSED."""
    ...
    assert "tempolimit_kmh muss gesetzt sein" in str(exc_info.value)


def test_construction_zone_tempolimit_required_for_lane_closed() -> None:
    """Validierung: tempolimit_kmh required für LANE_CLOSED."""
    with pytest.raises(ValidationError) as exc_info:
        ConstructionZone(
            betroffene_segmente=[0],
            tempolimit_kmh=None,
...
def test_construction_zone_tempolimit_required_for_reduced_lanes() -> None:
    """Validierung: tempolimit_kmh required für REDUCED_LANES."""
    with pytest.raises(ValidationError) as exc_info:
        ConstructionZone(
            betroffene_segmente=[0],
            tempolimit_kmh=None,
            sperrungstyp=Sperrungstyp.REDUCED_LANES,
            umleitungshinweis="Test",
            land=Land.DE,
            gueltig_von="2024-03-20T21:01:00+00:00",
            gueltig_bis="2024-03-21T03:00:00+00:00",
...
def test_construction_zone_tempolimit_not_required_for_full_closure() -> None:
    """Validierung: tempolimit_kmh nicht required für FULLY_CLOSED."""
    ...
    assert zone.sperrungstyp == Sperrungstyp.FULLY_CLOSED
    assert zone.tempolimit_kmh is None


def test_construction_zone_tempolimit_not_required_for_temp_speed_limit_with_value() -> None:
    """Validierung: tempolimit_kmh required für TEMPORARY_SPEED_LIMIT (mit Wert)."""
    zone = ConstructionZone(
        betroffene_segmente=[0],
        tempolimit_kmh=80,
        sperrungstyp=Sperrungstyp.TEMPORARY_SPEED_LIMIT,
...
def test_construction_zone_validation_max_tempolimit() -> None:
    """Validierung: tempolimit_kmh darf nicht > 200 sein."""
    with pytest.raises(ValidationError) as exc_info:
        ConstructionZone(
            betroffene_segmente=[0],
            tempolimit_kmh=201,
...
def test_construction_zone_validation_min_tempolimit() -> None:
    """Validierung: tempolimit_kmh darf nicht < 0 sein."""
    with pytest.raises(ValidationError) as exc_info:
        ConstructionZone(
            betroffene_segmente=[0],
            tempolimit_kmh=-1,
