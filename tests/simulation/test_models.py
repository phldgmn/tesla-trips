...
- Testfall 1: SimulationFrame Validierung (SoC-Bereich, Zustand-speed-Konsistenz)
...
class TestSimulationFrame:
    ...
    def test_valid_laden_frame(self) -> None:
        ...
        frame = SimulationFrame(
            timestamp=datetime(2026, 8, 15, 8, 30, 0, tzinfo=UTC),
            position=(52.5200, 13.4050),
            distance_m=1000.0,
            soc_pct=40.0,
    ...
    def test_valid_pause_frame(self) -> None:
        ...
        frame = SimulationFrame(
            timestamp=datetime(2026, 8, 15, 8, 15, 0, tzinfo=UTC),
            position=(52.5200, 13.4050),
            distance_m=500.0,
            soc_pct=60.0,
        ...
        assert frame.speed_kmh == 2.0

    def test_charging_frame_high_speed_raise(self) -> None:
        """Testfall 4: LADEN mit hoher speed muss ValueError werfen."""
        with pytest.raises(ValueError, match="Beim Laden muss speed"):
            SimulationFrame(
                timestamp=datetime(2026, 8, 15, 8, 0, 0, tzinfo=UTC),
                position=(52.5200, 13.4050),
                distance_m=0.0,
    ...
    def test_pause_frame_high_speed_raise(self) -> None:
        """Testfall 5: PAUSE mit hoher speed muss ValueError werfen."""
        with pytest.raises(ValueError, match="Bei Pause sollte speed"):
            SimulationFrame(
                timestamp=datetime(2026, 8, 15, 8, 0, 0, tzinfo=UTC),
                position=(52.5200, 13.4050),
                distance_m=0.0,
...
class TestChargingStopSummaryPricing:
    ...
    def _base_kwargs(self) -> dict[str, object]:
        return {
            ...
            "arrival_time": datetime(2026, 1, 1, 10, 0, tzinfo=UTC),
