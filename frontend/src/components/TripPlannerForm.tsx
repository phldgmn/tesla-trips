 ...
 *  wenn eine Zeit bekannt ist ("wo anwendbar", siehe Aufrufer). */
function Zeitbadge({
  kante,
  iso,
  socPct,
    ...
    >
      ...
      >
        {routeEintraege.map((eintrag, entryIdx) => {
          // Tageswechsel-Trenner: vergleicht den sortKey (Ankunft ?? Abfahrt)
          // dieses Eintrags mit dem des chronologisch vorherigen Eintrags
          // (siehe `istTageswechsel`/`route-eintraege.ts`).
          const vorherigeIso = routeEintraege[entryIdx - 1]?.sortKey ?? null;
          const aktuelleIso = eintrag.sortKey;
          const tagestrenner =
            istTageswechsel(vorherigeIso, aktuelleIso) &&
            vorherigeIso !== null &&
            aktuelleIso !== null ? (
              <Tagestrenner
                key={`trenner-${entryIdx}`}
                vorherigeIso={vorherigeIso}
                aktuelleIso={aktuelleIso}
              />
            ) : null;
          ...
          if (eintrag.art === "Stopp") {
            ...
            return (
              <Fragment key={stop.id}>
                {tagestrenner}
                <TimelineRow icon={Icon} background={background}>
                  <div
                    style={{
                      position: "relative",
                      border: "1px solid #e5e7eb",
                          ...
                          style={{
                            ...
                            cursor:
                              stops.length <= 2 || isSubmitting
                                ...
                                : "pointer",
                            opacity:
                              stops.length <= 2 || isSubmitting ? 0.5 : 1,
                          }}
                    ...
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        onBlur={() => {
                          ...
                          setTimeout(
                            () => handleCloseSuggestions(stop.id),
                            200,
                          );
                        }}
                        onFocus={(e) => {
          ...
          if (eintrag.art === "Ladehalt") {
            ...
            return (
              <Fragment key={stop.station_id}>
                {tagestrenner}
                <TimelineRow icon={Zap} background="#dcfce7">
                  <div
                    style={{
                      position: "relative",
                      border: "1px solid #e5e7eb",
          ...
          return (
            <Fragment key={key}>
              {tagestrenner}
              <TimelineRow icon={Ship} background="#dbeafe">
                <div
                  style={{
                    position: "relative",
