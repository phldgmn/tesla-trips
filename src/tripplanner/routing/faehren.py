...
def erkenne_faehren(route: Route) -> list[FerrySegment]:
    """Gruppiert zusammenhängende Fährsegmente einer Route zu `FerrySegment`-Einträgen.
    ...
    "Unbenannte Fähre"; Länge als Summe der `length_m`; Bounding Box aus allen
    ...
    Returns:
        Liste erkannter Fährverbindungen in heading. Leer, wenn die Route
...
def run_to_ferry_segment(run: list[RouteSegment]) -> FerrySegment:
    """Baut ein `FerrySegment` aus einem zusammenhängenden Lauf von Fähr-`RouteSegment`s."""
